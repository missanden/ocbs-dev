...
switch ($_GET['act']) {
    case "tampilschedule":
        ...
        while ($d = mysql_fetch_array($query3)) {
            mysql_query("DELETE FROM schedule_booking
                        WHERE 
                            kode_booking ='$d[kode_booking]'");
            mysql_query("UPDATE booking_mobil 
                        SET 
                            status_booking= 'remove' 
                        WHERE 
                            kode_booking ='$d[kode_booking]'");
        }
        ?>
        ...
        for ($x = 0; $x <= $y; $x++) {
            ...
            if ($_SESSION['s_position'] == 'admin') {
                ...
                if ($_SESSION['s_no_employee'] == '0902-085') { //Zaini Fikri
                    $qur = "SELECT * FROM jam_booking 
                            WHERE 
                                kode_jam BETWEEN '16' AND '34' 
                            ORDER BY 
                                jam_start ASC";
                    $q = "SELECT * FROM master_mobil
                        WHERE 
                            master_mobil.kepemilikan_mobil NOT IN ('Rental') AND 
                            master_mobil.aktif_mobil IN ('aktif') 
                        ORDER BY 
                            master_mobil.kepemilikan_mobil,master_mobil.no_polisi DESC";
                } else if ($_SESSION['s_no_employee'] == '0408-047') { //Anriga Kurniawan
                    $qur = "SELECT * FROM jam_booking 
                            WHERE 
                                kode_jam BETWEEN '16' AND '34' 
                            ORDER BY 
                    ...
                    $q = "SELECT * FROM master_mobil
                        WHERE 
                            master_mobil.kepemilikan_mobil IN ('Operasional','$_SESSION[s_no_employee]') AND
                    ...
                    $q = "SELECT * FROM master_mobil
                        WHERE 
                            master_mobil.kepemilikan_mobil='Operasional' AND 
                            master_mobil.aktif_mobil IN ('aktif') AND 
                            master_mobil.no_polisi NOT IN ('B1390FJD','B1340FIB','B1892FIH') 
                        ORDER BY 
                            master_mobil.kepemilikan_mobil,master_mobil.no_polisi DESC";
                }
			...
			<td width='7%'><font size='-2'><label> $d[type_mobil] : $d[nama_driver] </label></td>";
                    ...
                    while ($w = mysql_fetch_array($query4)) {
                        ...
                        $query = "SELECT * FROM schedule_booking 
                            INNER JOIN 
                                booking_mobil ON schedule_booking.kode_booking=booking_mobil.kode_booking
                            INNER JOIN 
                                master_employee ON master_employee.no_employee=booking_mobil.no_employee
                            INNER JOIN 
                                bonceng ON bonceng.kode_booking=booking_mobil.kode_booking
                            WHERE 
                                schedule_booking.kodeunik ='$kodeunik'";
                    ...
                    if ($day == '6' or $day == '7') {
                        ...
                        ////////////////////////////////////////////////////////////////////
                            ...
                            $q = "SELECT * FROM master_mobil 
                                WHERE
                                    kepemilikan_mobil='0408-047'";
                        } else if ($_SESSION['s_no_employee'] == '0401-001') { // Achmad Andreansyah
                            $qur = "SELECT * FROM jam_booking 
                                    WHERE 
                                        kode_jam BETWEEN '16' AND '34' 
                                    ORDER BY 
                        ...
                        } else if ($_SESSION['s_no_employee'] == '1403-572') { // Deby Trisnawati Dewi, SE., SS
                            $qur = "SELECT * FROM jam_booking 
                                    WHERE 
                                        kode_jam BETWEEN '16' AND '34' 
                                    ORDER BY 
                            ...
                            $q = "SELECT * FROM master_mobil 
                                    WHERE 
                                        kepemilikan_mobil='1403-572'";
                        } else if ($_SESSION['s_no_employee'] == '0809-083') { // Imam Mujahid
                            $qur = "SELECT * FROM jam_booking 
                                    WHERE 
                                        kode_jam BETWEEN '16' AND '34'
                                    ORDER BY 
                            ...
                            $q = "SELECT * FROM master_mobil 
                                    WHERE 
                                        kepemilikan_mobil='0809-083'";
                        } else if ($_SESSION['s_no_employee'] == '1207-163') { // Wahyu Andriyanto
                            $qur = "SELECT * FROM jam_booking 
                                    WHERE 
                                        kode_jam BETWEEN '16' AND '34' 
                                    ORDER BY 
                            ...
                            $q = "SELECT * FROM master_mobil
                                ...
                                ORDER BY 
                                    master_mobil.kepemilikan_mobil,master_mobil.no_polisi DESC";
                            /*
                             * Brio For marketing&sales Department
                             * need to change kepemilikan on table 'master_mobil' with value 'Marketing'
                             */
                            ...
                            $q = "SELECT * FROM master_mobil
                                WHERE 
                                    master_mobil.kepemilikan_mobil IN ('Operasional','Marketing') AND
                                    master_mobil.no_polisi NOT IN ('B1390FJD') AND
                                    master_mobil.aktif_mobil IN ('aktif') 
                                ORDER BY 
                                    master_mobil.kepemilikan_mobil,master_mobil.no_polisi DESC";
                        ...
                        while ($d = mysql_fetch_array($query3)) {
                            ...
                            while ($w = mysql_fetch_array($query4)) {
                                ...
                                $query = "SELECT * FROM  schedule_booking 
                                    INNER JOIN booking_mobil ON 
                                        schedule_booking.kode_booking=booking_mobil.kode_booking
                                    INNER JOIN master_employee ON 
                                        master_employee.no_employee=booking_mobil.no_employee
                                    INNER JOIN bonceng ON 
                                        bonceng.kode_booking=booking_mobil.kode_booking
                                    WHERE 
                                        schedule_booking.kodeunik ='$kodeunik'";
                                $query2 = mysql_query($query);
                                $row = mysql_num_rows($query2);
    ...
    case "tampilschedulewarehouse":
        ...
        for ($x = 0; $x <= 6; $x++) {
            ...
            if ($_SESSION['s_position'] == 'admin') {
                ...
                $q = "SELECT * FROM master_mobil
                    WHERE 
                        master_mobil.kepemilikan_mobil NOT IN ('Rental') 
                    ORDER BY 
                        master_mobil.no_polisi ASC";
            } else {
                $q = "SELECT * FROM master_mobil
                WHERE 
                    master_mobil.kepemilikan_mobil='Delivery' 
                ORDER BY 
                    master_mobil.no_polisi ASC";
            }
